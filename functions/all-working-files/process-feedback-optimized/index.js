...
async function logSuspiciousActivity(userId, ipAddress, reason, metadata = {}) {
  try {
    await tablesDB.createRow({
      ...
      tableId:
        process.env.SUSPICIOUS_ACTIVITY_COLLECTION_ID || 'suspiciousActivity',
      rowId: ID.unique(),
      data: {
        user_id: userId,
...
module.exports = async ({ req, res, log, error }) => {
  try {
    ...
    // ⚡ OPTIMIZATION: Run CAPTCHA validation and sanitization in PARALLEL
    log('Running parallel security checks (CAPTCHA + sanitization)...')
    const [captchaExecution, sanitizeExecution] = await Promise.all([
      // CAPTCHA validation (without rate limiting)
      functions.createExecution(
        ...
        JSON.stringify({
          ...
          skip_rate_limit: true, // Skip rate limiting for authenticated users
        }),
        false // synchronous
      ),
      // Input sanitization
    ...
    const captchaResult = JSON.parse(captchaExecution.responseBody || '{}')
    if (!captchaResult.success) {
      error(`CAPTCHA validation failed: ${captchaResult.error}`)
      return res.json(
        {
          success: false,
    ...
    log('✅ CAPTCHA validated successfully')

    // Parse sanitization result
    const sanitizeData = JSON.parse(sanitizeExecution.responseBody || '{}')
    if (!sanitizeData.success) {
      ...
      return res.json(
        {
          success: false,
          error: 'Invalid content in message',
        },
        400
      )
    }
    ...
    if (!sanitizedMessage) {
      ...
      error('Sanitized message is empty')
      return res.json(
        {
          success: false,
    ...
    // Log if dangerous content was detected and removed
    ...
    // Create feedback document (using TablesDB for future transaction support)
    ...
    const feedbackDoc = await tablesDB.createRow(
      ...
      [Permission.read(Role.user(user_id))] // User can read their own feedback
    )

    log(`✅ Feedback document created: ${feedbackDoc.$id}`)
